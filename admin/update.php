<?php
    ...
    if(!empty($_post))//c'est pas mon premier passage mais le deuxieme car on aurait appuyer sur le bouton modifier et donc le formulaire a ete soumis donc il a mis des choses dans la super globale $_post
    {
        ...
        if(($isSuccess && $isimageUpdate && $isUploadSuccess) || ($isSuccess && !$isimageUpdate))
        {
            $db=Database::connect();

            if($isimageUpdate)
            {
                $statement = $db->prepare("update items set name=?, description=?, price=?, category=?, image=? where items.id=?");
                $statement->execute(array($name, $description, $price, $category, $image, $id));
            }
            else
            {
                $statement = $db->prepare("update items set name=?, description=?, price=?, category=? where items.id=?");
        ...
        <div class="container admin">
            <div class="row">
                ...
                <div class="col-sm-6">
                    ...
                    <br>
                    <form action="<?php echo 'update.php?id=' . $id; ?>" method="post" role="form" class="form" enctype="multipart/form-data"><!--vue qu'on va uploader une image, on rajoute l'attribut enctype-->
                        ...
                        <div class="form-actions">
                            <button type="submit" class="btn btn-success"><i class="fa-sharp fa-solid fa-pencil"></i> Modifier </button>
                ...
                <div class="col-sm-6 site">
                    <div class="img-thumnbnail">
                        <img src="<?php echo '../images/'.$image; ?>" alt="" class="img-fluid">
